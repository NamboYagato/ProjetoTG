...
public class SecurityConfig {
    ...
    public SecurityFilterChain securityFilterChain(HttpSecurity http) throws Exception {
        http.csrf(AbstractHttpConfigurer::disable).sessionManagement(session -> session.sessionCreationPolicy(SessionCreationPolicy.STATELESS)).exceptionHandling(e -> e.authenticationEntryPoint((request, response, authException) -> {
            response.setStatus(HttpStatus.UNAUTHORIZED.value());
            response.setContentType("application/json");
            response.setCharacterEncoding("UTF-8");

            ErroDtoResponse erro = new ErroDtoResponse(authException.getMessage(), HttpStatus.UNAUTHORIZED.value());
            ...
            ErroDtoResponse erro = new ErroDtoResponse(accessDeniedException.getMessage(), HttpStatus.FORBIDDEN.value());
